...
/*
 * Print Estimated Channel Matrix
 */
void
print_csi_matrix(u8 csi_matrix[327],
                 u16 csi_matrix_len)
{
    int i, j;
    RTW_INFO("Print Channel Matrix:\n");
    for (i = 0; i < 16; i++) 
    {
        RTW_INFO("\t\t");
        for (j = 0; j < 20; j++)
            RTW_INFO("%d\t", csi_matrix[16 * i + j]);
        RTW_INFO("\n");
    }
    RTW_INFO("\t\t");
    for (j = 0; j < 7; j++)
        RTW_INFO("%d\t", csi_matrix[320 + j]);
    RTW_INFO("\n");
}
